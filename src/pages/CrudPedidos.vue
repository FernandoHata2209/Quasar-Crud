      ...
      <q-card>
        ...
        <q-card-section>
          <q-form @submit.prevent="updateProduct">
            <div class="row">
              <div class="col">
                <q-input v-model="formProduct.codigo" outlined label="Codigo" class="q-ma-md" required :rules="[
                  (val) =>
                    (val && val.length > 0) || 'Por favor preencher dados',
                ]"></q-input>
                <q-input v-model="formProduct.cliente" outlined label="Cliente" class="q-ma-md" required :rules="[
                  (val) =>
                    (val && val.length > 0) || 'Por favor preencher dados',
                ]"></q-input>
              </div>
              <div class="col">
                <q-input v-model="formProduct.entrega" outlined label="Data de Entrega" class="q-ma-md" :rules="[
                  (val) =>
                    (val && val.length > 0) || 'Por favor preencher dados',
                ]" required></q-input>
                <q-input v-model="formProduct.observacao" outlined label="Observação" class="q-ma-md" :rules="[
                  (val) =>
                    (val && val.length > 0) || 'Por favor preencher dados',
                ]" required></q-input>
              </div>
...
import QTableComponents from "../components/QTableComponents.vue";
